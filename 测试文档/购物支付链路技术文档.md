...
> **用户进入短视频/直播内容流 → 浏览商品讲解 → 点击商品卡 → 领券优惠或加购 → 提交订单 → 查看订单状态**
...
| **满减券** (full_reduction) | `selectedFullReductionCouponProvider` | 购物车底部"满减优惠"入口 | 整个购物车只能选 1 张 |
...
| min_order | REAL | 最低使用门槛（金额） | 100.00 |
| total_count | INTEGER | 总发行量 | 1000 |
| used_count | INTEGER | 已使用数量 | 0 |
| start_time | TEXT | 开始时间 | 2025-01-01T00:00:00Z |
| end_time | TEXT | 结束时间 | 2025-12-31T23:59:59Z |
...
| user_id | TEXT FK | 用户 ID | u1 |
| coupon_id | TEXT FK | 优惠券 ID | — |
| used | INTEGER | 是否已使用 (0/1) | 0 |
| created_at | TEXT | 领券时间 | — |
...
| `apps/mobile/lib/pages/order/order_confirm_page.dart` | 订单确认页（含优惠展示） |
...
| `apps/mobile/lib/models/coupon_model.dart` | 优惠券数据模型 |
| `apps/mobile/lib/provider/coupon_provider.dart` | 优惠券状态管理 |
| `apps/mobile/lib/widgets/coupon_card.dart` | 优惠券卡片组件 |
| `apps/mobile/lib/widgets/coupon_picker_sheet.dart` | 优惠券选择器弹窗 |
| `apps/mobile/lib/pages/mine/coupon_list_page.dart` | 个人中心优惠券列表页 |
